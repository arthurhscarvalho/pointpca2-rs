...
fn svd_sign_correction(mut u: na::DMatrix<f64>, mut v: na::DMatrix<f64>) -> (na::DMatrix<f64>, na::DMatrix<f64>) {
    let ncols = u.ncols();
    let nrows = u.nrows();
    ...
    let signs: Vec<f64> = max_abs_cols.iter()
        .enumerate()
...
pub fn compute_features<'a>(
    ...
    for i in 0..nrows {
        ...
        let projection_a_to_a = utils::subtract_row_from_matrix(&sl_points_a, &sl_points_a.row_mean()) * eigenvectors_a.clone();
        let projection_b_to_a = utils::subtract_row_from_matrix(&sl_points_b, &sl_points_a.row_mean()) * eigenvectors_a.clone();
        ...
        let mean_deviation_a = utils::subtract_row_from_matrix(&proj_colors_a_concat, &mean_a.clone());
        let mean_deviation_b = utils::subtract_row_from_matrix(&proj_colors_b_concat, &mean_b.clone());
        ...
        local_features.row_mut(i).view_mut((0, 0), (1, 3)).copy_from(&projection_a_to_a.row(0));
        local_features.row_mut(i).view_mut((0, 3), (1, 3)).copy_from(&projection_b_to_a.row(0));
        local_features.row_mut(i).view_mut((0, 6), (1, 3)).copy_from(&mean_a.view((0, 3), (1, 3)));
        local_features.row_mut(i).view_mut((0, 9), (1, 6)).copy_from(&mean_b);
        local_features.row_mut(i).view_mut((0, 15), (1, 6)).copy_from(&variance_a);
        local_features.row_mut(i).view_mut((0, 21), (1, 6)).copy_from(&variance_b);
        local_features.row_mut(i).view_mut((0, 27), (1, 6)).copy_from(&covariance_ab);
        local_features.row_mut(i).view_mut((0, 33), (1, 3)).copy_from(&eigenvectors_b.view((0, 0), (1, 3)));
        local_features.row_mut(i).view_mut((0, 36), (1, 3)).copy_from(&eigenvectors_b.view((1, 0), (1, 3)));
        local_features.row_mut(i).view_mut((0, 39), (1, 3)).copy_from(&eigenvectors_b.view((2, 0), (1, 3)));
    }
